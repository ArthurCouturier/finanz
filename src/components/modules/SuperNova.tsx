...
import { totalWeeklySales, mealsCookedPerWeek, workedDaysPerWeek, workedMealsPerWeek, getAverageBasketPerWeek } from "@/modules/StatisticsPerWeek";
...
export default function SuperNova({
    ...
    return (
        <div className="flex justify-center">
            {showDetails ? 
                <Card className="mx-4 mt-2 pt-2 w-full h-full p-4" onClick={() => setShowDetails(false)}>
                    ...
                    >
                        <div className="flex flex-col items-center">
                            ...
                            <div className="w-full flex justify-center items-center">
                                <NumberFlow value={annualSales} className="text-xl font-bold mx-1" />
                                <p className="font-bold text-xl">€</p>
                            </div>
                        </div>
                        <div className="flex flex-col items-center">
                            <p className="text-xl max-w-64">Avg. table settings per working day :</p>
                            <NumberFlow value={parseFloat(averageMealsPerDay)} className="text-xl font-bold" />
                        </div>
                        <div className="flex flex-col items-center max-w-64">
                            <p className="text-xl">Avg. table settings per meal worked :</p>
                            <NumberFlow value={parseFloat(averageMealsPerMeal)} className="text-xl font-bold" />
                        </div>
                        <div className="flex flex-col items-center">
                            <p className="text-xl">Number of days per year :</p>
                            <NumberFlow value={workedDays} className="text-xl font-bold" />
                        </div>
                        <div className="flex flex-col items-center">
                            <p className="text-xl">Table settings per year :</p>
                            <NumberFlow value={mealsCooked} className="text-xl font-bold" />
                        </div>
                        <div className="flex flex-col items-center">
                            <p className="text-xl">Unit sale price (excl. tax) :</p>
                            <div className="flex items-center">
                                <NumberFlow value={parseFloat(averageBasket)} className="text-xl font-bold" />
                                <p className="font-bold text-xl">€</p>
                            </div>
                        </div>
                    </div>
