...
import { ExportConfigurationButton } from "../buttons/DataImportButton";
...
export default function HouseConfiguration({
  ...
  const [annualInterestRate, setAnnualInterestRate] = useState(
    config.annualInterestRate.value
  );
  const [loanTermMonths, setLoanTermMonths] = useState(
    config.loanTermMonths.value
  );
  const [preferLoanYear, setPreferLoanYear] = useState(config.preferLoanYear);
  const [annualInsuranceRate, setAnnualInsuranceRate] = useState(
    config.annualInsuranceRate.value
  );
  ...
  const [monthlyInsurancePayment, setMonthlyInsurancePayment] = useState(
    compute().monthlyInsurancePayment
  );
  const [totalRepayment, setTotalRepayment] = useState(
    compute().totalRepayment
  );
  const [totalInterestCost, setTotalInterestCost] = useState(
    compute().totalInterestCost
  );
  ...
  return (
    <Card className="h-fit">
      ...
      <CardContent className="flex justify-center">
        <Switch
          className="mx-4"
          checked={preferLoanYear}
          onCheckedChange={setPreferLoanYear}
        />
        <p>Pref. Years</p>
        <Switch
          className="mx-4"
          checked={inclInsurance}
          onCheckedChange={setInclInsurance}
        />
        <p>Incl. Insurance</p>
        <Switch
          className="mx-4"
          checked={decimal}
          onCheckedChange={setDecimal}
        />
        <p>Decimals</p>
      </CardContent>
      ...
      <CardContent className="grid grid-cols-2 w-fit mx-auto">
        <CardContent>
          <p>Total Repayment:</p>
          <NumberFlow
            value={parseFloat(totalRepayment.toFixed(decimal ? 2 : 0))}
        ...
        <CardContent>
          <p>Monthly Payment:</p>
          <NumberFlow
            value={parseFloat(monthlyPayment.toFixed(decimal ? 2 : 0))}
            className="text-2xl font-bold"
          />
        </CardContent>
        <CardContent>
          ...
          <NumberFlow
            value={parseFloat(totalInterestCost.toFixed(decimal ? 2 : 0))}
            className="text-2xl font-bold"
          />
        </CardContent>
        <CardContent>
          ...
          <NumberFlow
            value={parseFloat(monthlyInsurancePayment.toFixed(decimal ? 2 : 0))}
            className="text-2xl font-bold"
          />
        </CardContent>
      </CardContent>
      <CardFooter className="flex items-center justify-center gap-6 mt-4">
        ...
        <ExportConfigurationButton uuid={config.uuid} name={name} />
      </CardFooter>
    </Card>
  );
}
